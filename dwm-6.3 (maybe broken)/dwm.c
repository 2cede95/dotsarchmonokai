...
/*static void setgaps(const Arg *arg); (used for adjusting gaps)*/
	...
	for (i = 0, my = ty = m->gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			if(h < minwsz) {
				c->isfloating = True;
				XRaiseWindow(dpy, c->win);
				resize(c, m->mx + (m->mw / 2 - WIDTH(c) / 2) + m->gappx, m->my + (m->mh / 2 - HEIGHT(c) / 2), m->ww - mw - (2*c->bw) - m->gappx, h - (2*c->bw), False);
				ty -= HEIGHT(c) + m->gappx;
			}
			else
